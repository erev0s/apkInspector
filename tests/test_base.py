...
from apkInspector.headers import show_and_save_info_of_headers, EndOfCentralDirectoryRecord, CentralDirectory, ZipEntry
...
class ApkInspectorTestCase(unittest.TestCase):
    ...
    def test_find_eocd(self):
        ...
        orig = {'signature': b'PK\x05\x06', 'number_of_this_disk': 0, 'disk_where_central_directory_starts': 0, 'number_of_central_directory_records_on_this_disk': 23, 'total_number_of_central_directory_records': 23, 'size_of_central_directory': 1317, 'offset_of_start_of_central_directory': 151552, 'comment_length': 0, 'comment': ''}
        mod = {'signature': b'PK\x05\x06', 'number_of_this_disk': 0, 'disk_where_central_directory_starts': 0, 'number_of_central_directory_records_on_this_disk': 24, 'total_number_of_central_directory_records': 24, 'size_of_central_directory': 1420, 'offset_of_start_of_central_directory': 151552, 'comment_length': 0, 'comment': ''}
    ...
    def test_parse_local_header(self):
        ...
        comp_orig = {'version_needed_to_extract': 20, 'general_purpose_bit_flag': 0, 'compression_method': 0, 'file_last_modification_time': 2081, 'file_last_modification_date': 545, 'crc32_of_uncompressed_data': 998929674, 'compressed_size': 2084, 'uncompressed_size': 2084, 'file_name_length': 19, 'extra_field_length': 3, 'filename': 'AndroidManifest.xml', 'extra_field': '\x00\x00\x00'}
        comp_mod = {'version_needed_to_extract': 20, 'general_purpose_bit_flag': 0, 'compression_method': 30208, 'file_last_modification_time': 45686, 'file_last_modification_date': 22321, 'crc32_of_uncompressed_data': 2768741858, 'compressed_size': 2084, 'uncompressed_size': 2084, 'file_name_length': 19, 'extra_field_length': 0, 'filename': 'AndroidManifest.xml', 'extra_field': ''}
    ...
    def test_extract_file_based_on_header_info(self):
        ...
        androidManifest_extracted = extract_file_based_on_header_info(self.apk_orig, local_header_of_file, cd_h_of_file)[0]
    ...
    def test_extract_modified_file_based_on_header_info(self):
        ...
        androidManifest_extracted = extract_file_based_on_header_info(self.apk_mod, local_header_of_file, cd_h_of_file)[0]
        self.assertEqual(hashlib.sha256(androidManifest_extracted).hexdigest(), ext_hash)
    ...
    def test_tampering_indicators_size(self):
        test_dir = os.path.dirname(os.path.abspath(__file__))
        expected = {'zip tampering': {'AndroidManifest.xml': {'differing headers': ['compressed_size', 'extra_field', 'extra_field_length', 'uncompressed_size']}}, 'manifest tampering': {}}
        with open(os.path.join(test_dir, 'res', 'minimal_SIZE_ExtraField.apk'), 'rb') as apk_file:
